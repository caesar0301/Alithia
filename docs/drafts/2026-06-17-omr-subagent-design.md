...
Add `OmrAgentConfig` to `alithia_agent/config/schema.py`:
...
```python
# alithia_agent/omr/__init__.py

from soothe_sdk.plugin import plugin, subagent
from alithia_agent.omr.implementation import create_omr_subagent
from alithia_agent.omr.state import OmrRuntimeConfig, AgentState
...
Add to `alithia_agent/plugin_registration.py`:
...
1. Create package structure (`alithia_agent/omr/`)

...
This document defines the process, conventions, and guidelines for creating and managing RFCs (Requests for Comments) in the alithia-agent project.
...
RFCs are the authoritative specifications for the alithia-agent system. They define:
...
alithia-agent RFCs fall into three kinds, each with a distinct purpose and level of abstraction:
...
**Stage**: [Optional: e.g. Core | Storage | API | Infra]
...
**For questions about RFC management, contact the alithia-agent core team.**
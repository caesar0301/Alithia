...
# Environment variable resolution (local implementation)
...
def resolve_deepxiv_token(token: str | None) -> str | None:
    """Resolve DeepXiv API token from config or env (``DEEPXIV_API_KEY``/``DEEPXIV_TOKEN``)."""
    if token:
    ...
    def reader(self) -> Any:
        ...
        if self._reader is None:
            try:
                ...
                logger.warning(
                    "[DeepXiv] deepxiv_sdk not installed; "
                    "install deepxiv-sdk package for academic tools"
                )
                raise RuntimeError(
                    "deepxiv_sdk not installed. Install with: pip install deepxiv-sdk"
                )
        return self._reader
